...
def main (*args):
    ...
    time0 = time.time()
    with cuda.pinned(callResult, putResult):

        blockdim = 1024, 1
        griddim = int(math.ceil(float(OPT_N)/blockdim[0])), 1
        stream = cuda.stream()

        d_callResult = cuda.to_device(callResult, stream)
        d_putResult = cuda.to_device(putResult, stream)
        ...
        for i in range(iterations):
            ...
            d_callResult.copy_to_host(callResult, stream)
            d_putResult.copy_to_host(putResult, stream)
            stream.synchronize()
